...
message = tk.Label(window, text="Enter Details", width=30,
                   height=3, font=('times', 30, ' bold '))
message.place(x=400, y=20)

lbl = tk.Label(window, text="Enter ID", width=20,
...
lbl2 = tk.Label(window, text="Enter Name", width=20,
                height=2, font=('times', 15, ' bold '))
    ...
    while(True):
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.imwrite("dataSet/User." + str(id) + "." +
                        str(imageNum) + ".jpg", gray[y: y + h, x: x + w])
...
def trackImages():
    ...
    while(True):
        ...
        faces = faceDetect.detectMultiScale(
            gray, 1.2, 5, flags=cv2.CASCADE_SCALE_IMAGE)
        ...
        for (x, y, w, h) in faces:
            ...
            if (profile != None):
                if(conf < 100):
                    ...
                    cv2.putText(frame, '%s - %.0f' % (str(
                        profile[1]), 100 - conf), (x - 10, y - 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
                else:
                    cv2.putText(frame, str('unknown'), (x - 10, y - 10),
...
clearButton = tk.Button(window, text="Clear", command=clear,
                        width=10, height=1, font=('times', 15, ' bold '))
...
takeImg = tk.Button(window, text="Take Images", command=takeImages,
                    width=20, height=1, font=('times', 15, ' bold '))
